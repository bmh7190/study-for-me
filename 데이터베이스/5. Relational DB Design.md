...
어떤 관계 스키마 `R`이 **제1정규형**에 있다는 것은,  `R`의 모든 속성이 **원자적 도메인을 가진다**는 것을 의미한다. **도메인(domain)** 이 **원자적(atomic)** 이라는 것은,  그 도메인의 각 요소가 **더 이상 나눌 수 없는 단위**로 간주된다는 의미이다.

atomic 하지 않게 저장된 값이라면 저장이 복잡해지고, 중복 저장이 될 수 도 있다. 


**원자성(Atomicity)은 도메인의 사용 방식에 따라 달라진다**

- 도메인이 원자적인지 여부는 **그 값이 어떻게 사용되는지**에 따라 달라진다.
    
- 예를 들어, 문자열(string)은 일반적으로 **더 이상 나눌 수 없는 원자값**으로 간주된다.