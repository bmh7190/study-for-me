...
control filed는 이 TCP 세그먼트가 어떤 성격을 띄고 있는지 나타내는 필드로 6개 비트로 이뤄져있다. 중요한 것부터 보자면 ACK은 말 그대로 이 segment가 ack 을 가지고 있는지 없는지를 나타내고, SYN은 클라이언트와 서버가 연결할 때, 연결 요청을 보내는 segment라는 것을 알려주고, FIN은 연결 종료 요청을 보내는 segment라는 것을 알려준다. 
