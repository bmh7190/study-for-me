...
그래서 이렇게 하나의 커넥션에서 여러개의 흐름으로 나눠서 네트워크를 점유하는 것을 p2p 라고 한다. 전체 네트워크 전체 용량 중 p2p로 여러개 차지 하게 되면, 제대로 하나만 쓰는 사용자는 영향을 많이 받아 네트워크 속도가 저하될 것이다. 단순히 가정집에서는 문제가 안 될 수 있지만 기숙사에서 여러 명이 동일 네트워크에 접속하는 경우에는 문제가 될 수 있다.
...
TCP는 setup과정이 신뢰성을 보장하면 flow control을 하고 congestion control를 통해 혼잡을 제어했다. 그래서 TCP로 온 데이터들을 Application 은 그냥 믿고 쓰기만 하면 된다. 하지만 UDP는 이 4가지를 전부 하지 않는 차이점이 있다. 
