...
이 경우는 서브넷을 나눈 것이 아니라, 서로 비슷한 주소 범위를 가진 여러 네트워크를 하나로 묶어서 라우팅 테이블에 기록하는 **주소 집약(route aggregation)** 또는 **prefix aggregation**의 예시이다. 여러 목적지 네트워크의 앞부분 비트가 모두 동일하고, 그 네트워크들이 동일한 방향(즉, 같은 인터페이스)으로 전달된다면, 각각을 개별 엔트리로 기록할 필요가 없다. 이럴 때 라우터는 공통된 prefix를 사용해 더 큰 하나의 네트워크 주소로 병합할 수 있고, 이는 라우팅 테이블을 더 간단하고 효율적으로 만든다.
