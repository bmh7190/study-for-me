Distance Vector와는 달리, 인트라도메인에서 사용되는 또 다른 방식인 **Link State Routing**은 네트워크를 바라보는 관점 자체가 다르다. 이 방식에서는 각 라우터가 자신과 _직접 연결된 링크_에 대한 정보만큼은 정확히 알고 있다고 가정한다. 즉, “내가 누구와 연결되어 있고 비용은 얼마인지”에 대한 로컬 정보는 확실하다.
