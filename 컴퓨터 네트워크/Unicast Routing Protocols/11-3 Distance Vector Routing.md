...
예를 들어 A–B–C 세 라우터가 있고, A는 X까지의 실제 경로를 알고 있다고 하자. 이때 X와 A의 연결이 끊어지면 A는 이를 B와 C 양쪽에 전파한다. B는 정상적으로 업데이트를 받았지만, C로 가는 패킷이 유실되었다고 가정하면서 문제가 시작된다.
