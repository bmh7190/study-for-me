...
또 다른 예시를 보자. 지금 170.14.5.165에서 192.16.7.5 로 전송하고자 한다. 
...
이러한 과정 덕분에 패킷은 R1을 떠나 111.15.17.32 라우터로 전달되며, 이후 R3를 거쳐 최종 목적지인 192.16.7.5까지 도달하게 된다.
...
이후 자신이 알고 있는 서브넷 마스크(/18)를 적용하여 목적지 주소가 속한 **서브넷**을 계산한다. 서브넷 마스크가 /18이라는 것은 주소의 앞 18비트가 네트워크 부분이라는 의미이며, 나머지 14비트는 호스트 부분이 된다. 즉, 라우터는 목적지 주소의 처음 18비트만 보고 어떤 서브넷에 속하는지를 판단할 수 있다.
...
#### Example 2
목적지 주소가 **201.4.22.35**인 패킷이 라우터에 도착하면, 라우터는 라우팅 테이블의 엔트리들을 위에서부터 순서대로 보면서 각 엔트리에 적힌 마스크를 적용해본다. 패킷 안에는 목적지 주소만 있을 뿐 마스크 정보는 포함되어 있지 않기 때문에, 어떤 서브넷에 속하는지는 라우터가 테이블의 mask 값을 이용해 직접 계산해야 한다.

먼저 가장 긴 prefix인 **/26** 마스크를 적용해 보면, 201.4.22.35는 201.4.22.192 또는 201.4.22.128과 같은 /26 네트워크 중 어느 것과도 일치하지 않는다. 따라서 /26 엔트리는 사용할 수 없다. 다음으로 **/25** 마스크를 적용해 보지만 결과는 이 경우도 네트워크 주소와 일치하지 않는다.

그다음으로 **/24** 마스크를 적용하면, 201.4.22.35는 201.4.22.0/24 네트워크에 속한다는 것을 알 수 있다. 라우팅 테이블에 있는 /24 항목의 네트워크 주소가 **201.4.22.0**이므로 정확히 일치하며, 이는 가장 긴 prefix 중에서 처음으로 매칭되는 엔트리이다. 따라서 라우터는 이 엔트리를 선택해 패킷을 **m3 인터페이스**로 전달하게 된다.

이처럼 라우터는 항상 긴 프리픽스(mask가 큰 값)부터 적용해보고, 계산된 네트워크 주소가 테이블의 네트워크 주소와 동일할 때 해당 경로를 선택한다. 이번 예시도 이런 기본 원리에 따라 /24 엔트리가 선택되는 것이다.

#### Example 3
18.24.32.78인 경우에 지금 라우팅 테이블에 있는 mask 를 다 적용 해도 맞는게 없다 그럴 경우에 default로 보낸다.

---
## Guessed topology

![](../../images/Pasted%20image%2020251204225951.png)

---
## Address Aggregation

이 경우는 서브넷을 나눈 것이 아니라, 서로 비슷한 주소 범위를 가진 여러 네트워크를 하나로 묶어서 라우팅 테이블에 기록하는 **route aggregation** 또는 **prefix aggregation**의 예시이다. 여러 목적지 네트워크의 앞부분 비트가 모두 동일하고, 그 네트워크들이 동일한 방향(즉, 같은 인터페이스)으로 전달된다면, 각각을 개별 엔트리로 기록할 필요가 없다. 이럴 때 라우터는 공통된 prefix를 사용해 더 큰 하나의 네트워크 주소로 병합할 수 있고, 이는 라우팅 테이블을 더 간단하고 효율적으로 만든다.

![](../../images/Pasted%20image%2020251204230427.png)

하지만 중요한 조건이 하나 있다. **병합하려는 네트워크들이 모두 동일한 인터페이스로 나가야 한다는 것**이다. 만약 두 네트워크의 목적지가 서로 다른 인터페이스를 향한다면, 하나의 엔트리로 묶어버릴 수 없다. 이유는 간단하다. 만약 라우터가 병합된 큰 prefix 엔트리를 먼저 매칭해 버리면, 원래 다른 방향으로 가야 한다는 세부 경로가 무시되기 때문이다.

이런 상황에서는 아래와 같이 처리한다.  

![](../../images/Pasted%20image%2020251204231010.png)

공통 prefix를 묶어서 상위 네트워크를 만들되, **특정 세부 네트워크에 대해 다른 방향으로 가야 하는 경우에는 그 네트워크를 더 길고 구체적인 mask(/n이 더 큼)로 별도로 추가**한다. 라우팅 테이블은 항상 **Longest Prefix Match**, 즉 가장 긴 mask를 먼저 검사하기 때문에, 인터페이스가 다른 예외적인 네트워크는 자연스럽게 정확한 방향으로 보내지고, 그 외의 일반적인 주소들은 병합된 큰 prefix로 처리된다.

즉, 인터페이스가 다른 네트워크가 섞여 있어도 문제는 없다. **예외는 mask를 더 길게 해서 따로 기록하고, 나머지는 병합해서 관리하면 된다.** 라우팅 테이블은 prefix가 긴 항목부터 매칭하기 때문에 충돌 없이 올바른 방향으로 패킷을 전달할 수 있다.