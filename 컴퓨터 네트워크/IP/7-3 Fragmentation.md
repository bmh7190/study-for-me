데이터그램은 여러 종류의 네트워크를 거쳐 전달될 수 있다. 각 라우터는 먼저 자신이 받은 프레임에서 **IP 데이터그램을 디캡슐레이션(decapsulation)** 하여 꺼낸 뒤, IP 헤더를 검사하고 필요한 처리를 수행한다. 이후 해당 데이터그램을 다음 네트워크로 보내기 위해 **새로운 프레임으로 다시 캡슐레이션(encapsulation)** 한다.
