...
`fixHeap(H, K)`는 힙을 유지하기 위해 새로운 원소 K를 트리 HHH에 삽입하는 절차를 나타낸다.  
이 함수는 재귀적으로 동작하며, 트리의 높이 h에 따라 최대 h번까지 재귀 호출이 일어날 수 있다.

먼저, H가 리프 노드이면, KKK를 해당 루트 위치에 삽입하고 종료한다.  그렇지 않으면, 왼쪽 서브트리와 오른쪽 서브트리의 루트 키 값을 비교하여,  더 큰 키를 가진 서브트리를 `largerSubHeap`으로 선택한다.  
이때 자식들 간의 비교 1회가 발생한다.

이후, K와 `largerSubHeap`의 루트 키를 비교한다.  만약 K의 키가 더 크거나 같으면, K를 현재 루트에 삽입한다.  반대로, K가 더 작으면 `largerSubHeap`의 루트 키를 현재 루트에 올리고,  K를 해당 서브트리에 재귀적으로 삽입하기 위해 `fixHeap(largerSubHeap, K)`를 호출한다.
