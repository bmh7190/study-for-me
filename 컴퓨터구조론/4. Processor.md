...
사전에 multiple issue를 미리 정해놓는게 아니라 실시간으로 프로세서, 하드웨어, CPU가 정하는 것이다.
이렇게 speculation을 통해 여러 instruction 재배치를 해서 한꺼번에 여러개 instruction을 실행하는 프로세서를 Superscalar 프로세서라고 한다. 

당연히 structural 혹은 data hazard를 피해야한다. 

컴파일러가 미리 스케쥴링하는게 크게 필요하지 않다. 도움은 될 수 있어도,,, 그리고 CPU가 옳은 동작을 보장하는지 코드가 semantic해야한다.