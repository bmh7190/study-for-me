...
두 번째 방법은 **동시에 여러 instruction을 동시에 발행**하는 것이다. 이를 위해 **공유해야 하는 자원을 제외한 나머지 파이프라인 구조를 복제**한다.

- 즉, **파이프라인 스테이지를 복제하여 다중 파이프라인 구조를 만들고**,
    
- **하나의 clock cycle마다 여러 개의 instruction을 시작**하게 하는 것이다.
    

이 경우 CPI는 1보다 작아질 수 있기 때문에, **Instructions Per Cycle (IPC)**를 대신 사용한다.

예를 들어, 4GHz 클럭에서 4-way multiple-issue 구조라면,
...
PC, PC+4를 항상 같이 fetch 이렇게 하면 근데 2 instruction 사이의 의존성이 없어야 한다. 이 의존성은 컴파일러가 instruction 재배치를 통해 해결한다. 전에는 PC, PC+4, PC+8에 하나씩 배치했다면, 이제는 PC, PC+4를 하나로 패키징하고 묶어놓은 것은 issue slots이라고 한다.

dynamic multiple issue
CPU가 여러개의 instruction을 여러개 fetch하고 그중에 상관 없;는 걸들을 모아서 같이 issue
