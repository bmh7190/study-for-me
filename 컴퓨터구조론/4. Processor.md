...
pipelining : 여러 instruction을 병렬적으로 동시에 실행하도록 한다.

여러 instruction이 병렬적으로 돌아가는 특성을 instruction level parallelism이 있다고 한다.

ILP를 높이는 것은 system 처리량을 높이는 방법 중 하나다.

ILP를 높이기 위해서는 파이파라인을 깊게 만들면 할 수 있다. 전체 스테이지를 여러 스테이지로 줄이는 일이기 때문에, 한 스테이지 당 해야 하는 일을 적게 하기 때문에 clock cycle 도 줄어든다.

두번째 방법은 동시에 여러 instruction을 동시에 발행

공유해야 하는 자원을 제외하고 파이프라인을 복제한다. 
