...
지지금 다루고 있는 MIPS 구조에서는 이러한 문제가 발생하지 않는다. 
...
고정된 방식으로 예측하는 방법으로, **일반적인 분기 패턴**에 기반하여 판단한다.  예를 들어 반복문에서는 **보통 루프를 다시 반복하는 경우가 많기 때문에** backward jump는 taken, forward jump는 not taken으로 정해놓고 예측한다.  이 방식은 **구현이 단순하고 어느 정도 효과가 있지만**, 다양한 상황에서 높은 정확도를 기대하긴 어렵다.
...
MEM 결과물이 ALU 상단으로 들어갈 수도 있고, ALU 하단으로 들어갈 수도 있다.  마찬가지로 WB 결과물이 ALU 상단이나 하단으로 들어가는 경우도 있을 수 있다.
...
`beq` 명령어의 비교 연산을 더 빠르게 수행하기 위해, ID 단계에 비교용 소형 ALU가 있어 이 단계에서 비교가 이루어진다. 이때 비교에 사용되는 `$1`, `$4` 레지스터가 직전 명령어의 결과로 같은 시점(ID 단계)에 필요하다면, 아직 값이 준비되지 않았기 때문에 어쩔 수 없이 한 사이클의 stall이 발생한다.
...
이를 위해 **Branch Prediction Buffer** 또는 **Branch History Table**이라 불리는 구조를 사용한다.  이 테이블은 **최근에 실행된 branch instruction의 주소를 인덱스로 사용**하며, 그 명령어의 **과거 분기 결과(taken / not taken)** 를 저장하고 있다. 모든 주소를 저장하는 것은 아니고, 일부 주소만 저장한다. 
...
이러한 **추정 기반 실행(speculative execution)** 은 성능을 높이기 위해 널리 사용되며,  추정이 성공할 경우 처리 속도를 높일 수 있지만, **실패 시에는 되돌릴 수 있는 메커니즘이 반드시 필요**하다.
...
이론적으로는 한 번에 두 개의 명령어를 실행할 수 있어 효율성이 두 배로 증가할 것처럼 보이지만, 실제로는 병렬 실행에 따른 hazard 관리가 훨씬 더 복잡해진다.

예를 들어 single-issue 구조에서는 forwarding을 통해 대부분의 stall을 피할 수 있다. 하지만 dual-issue 구조에서는 같은 패킷 내에서 ALU 결과를 바로 load/store에 사용할 수 없어 문제가 생긴다.
...
위와 같이 `add`의 결과를 바로 다음 명령어인 `load`에서 사용하는 경우, 같은 패킷에 함께 넣을 수 없기 때문에 둘을 분리해야 하고, 이는 사실상 한 사이클 stall을 의미한다.

특히 load-use hazard에서는 한 사이클을 nop으로 채워야 하고, 이때는 병렬 실행의 장점을 살리지 못해 결과적으로 명령어 두 개를 실행하지 못하는 손해가 발생하므로, 효율성 측면에서 2배 손해를 본 셈이 된다.

따라서 이런 구조에서는 순서 재배치 같은 사전적인 hazard 회피 기법이 반드시 필요하며, 보다 aggressive한 명령어 스케줄링이 요구된다.
...
위 두 명령어는 동시에 실행할 수 없다. `lw`는 `$t0`에 값을 로드해야 하는데, 아직 MEM 단계에서 값이 나오지 않은 상태에서 바로 아래 `addu`에서 `$t0` 값을 사용하려고 하기 때문이다. 이는 **load-use hazard**로 인해 같은 issue packet에 넣을 수 없다.
...
이 두 명령어도 동시에 실행할 수 없다. `bne` 명령어가 정확한 분기 판단을 하려면 **`addi`의 결과가 반영된 이후에야 실행 가능**하기 때문이다. 즉, `$s1` 값이 갱신되기 전에는 올바른 분기 여부를 알 수 없다.

이러한 제약 조건들 때문에 instruction을 2개씩 묶고, **같이 issue할 수 없는 경우에는 `nop`을 채워 넣게 되며**,  결과적으로 다음과 같은 형태의 instruction 스케줄이 만들어질 수 있다:


![](../images/Pasted%20image%2020250526161934.png)
...
Loop body를 복제하면 더 많은 병렬성을 드러낼 수 있다. 이를 통해 **loop-control overhead(루프 제어 비용)** 도 줄일 수 있다. ( beq )

단, 복제된 loop body는 **각 복제마다 다른 레지스터**를 사용해야 한다. 이 기법을 **register renaming(레지스터 이름 바꾸기)** 라고 한다.  그 목적은 루프 간 위반되는 의존성을 피하기 위함이다.

예를 들어, 어떤 루프에서 한 iteration에서 **store를 수행하고**, 다음 iteration에서 **같은 레지스터를 load**하려는 경우가 있다.  이처럼 **같은 레지스터 이름의 재사용으로 인해 발생하는 의존성**을  **“name dependence(이름 의존성)”** 이라고 부르며,  이를 피하려면 **레지스터 이름을 다르게 사용해 충돌을 방지**해야 한다.

![](../images/Pasted%20image%2020250526171025.png)
...
원래는 loop마다 주소를 `-4`씩 감소시켰지만, **loop unrolling을 통해 4개의 연산을 한 번에 수행**하기 때문에,  **초기에 한 번에 `-16`을 빼주는 방식으로 조정**한 것이다.  
즉, "4번 `addi $s1, $s1, -4`를 할 것을 한 번에 미리 계산해준 것"으로 보면 된다.
...
이 두 instruction은 **load-use hazard** 때문에 원래는 **한 cycle을 비우고(add delay)** `addu`를 실행해야 했다.  하지만 이렇게 하면 **중간에 nop이 삽입되어 자원이 낭비**되기 쉽다.

그런데 **loop를 unroll해서 같은 작업을 여러 번 반복**하도록 하면,  이런 **빈 공간(nop이 들어갈 자리)을 다른 유사한 instruction으로 채울 수 있다.**


> [!note] Unrolling이 무조건 좋은가?
> 이론적으로는 최대 IPC가 2일 수 있지만, 이 예제에서는 **1.75로 매우 근접**한 값을 달성했다.
> 다만, 이렇게 높은 IPC를 얻기 위해서는 **다음과 같은 비용이 따른다**:
> - **레지스터 사용 증가**:  
>     loop unrolling으로 인해 `$t0, $t1, $t2, $t3, ...` 등 더 많은 레지스터가 필요하다.
>
> - **코드 크기 증가**:  
>     반복문을 복제하므로 instruction 수가 늘어나 **코드 사이즈가 커진다.** 즉 프로그램 용량이 커진다.
> - **Loop size에 따른 조건**을 더 철저하게 확인해야 한다. 남는 instruction이 없도록!

---
## **Dynamic Multiple Issue**

사전에 multiple issue를 미리 정해놓는 것이 아니라, **실시간으로 프로세서(하드웨어, CPU)가 instruction을 동적으로 분석하고 결정**하는 방식이다.  이렇게 **speculation과 동적 재배치(dynamic scheduling)** 를 통해 **한 번에 여러 instruction을 실행하는 프로세서**를 **Superscalar 프로세서**라고 한다.

이런 구조에서는 당연히 **structural hazard**(자원 충돌)나 **data hazard**(데이터 의존성)를 **정확히 피하고 관리해야 한다.**

Superscalar 방식에서는 **컴파일러가 instruction을 미리 스케줄링할 필요가 크지 않다.** 물론 **도움이 될 수는 있지만**, 핵심 스케줄링과 hazard 관리는 **CPU 내부에서 수행된다.**

따라서 **CPU가 실행 순서를 유연하게 조정하더라도, 최종 실행 결과는 원래 프로그램의 의미(semantic)를 반드시 유지해야 한다.**  즉, **동작의 정확성은 CPU가 보장해야 하며**, 최적화된 실행이 이루어지더라도 **코드는 논리적으로 일관되어야 한다.**

---
## **Dynamic Pipeline Scheduling**

CPU가 **stall을 피하기 위해 instruction을 out-of-order로 실행하는 것을 허용**한다면,  예상보다 **더 많은 명령어를 병렬로 실행할 수 있게 되어 성능이 향상**된다.

하지만 **최종적으로 레지스터에 결과를 반영(commit)하는 순서는 반드시 프로그램에 정의된 순서대로** 이루어져야 한다.
...
- `addu`는 `$t0`의 값이 필요하므로 `lw`가 끝날 때까지 기다려야 한다.
    
- 하지만 그동안 **`sub`는 `$t0`와 관계가 없으므로 먼저 실행 가능**하다.
    
- 마찬가지로 `slti`도 `sub`의 결과만 기다리면 되므로 순차적이지 않게 진행될 수 있다.
    

이처럼 **실행은 out-of-order로 하되**,  **결과를 레지스터에 쓰는(commit) 순서는 반드시 in-order**로 유지함으로써  **병렬성과 정확성을 모두 확보할 수 있다.**


---
## **Dynamically Scheduled CPU**

![](../images/Pasted%20image%2020250527231624.png)

Reservation station과 reorder buffer는 **사실상 register renaming을 구현하는 구조**다.

**Instruction이 reservation station에 issue될 때**,  필요한 operand(피연산자)를 확인한다.
만약 operand가 이미 **레지스터 파일**이나 **reorder buffer**에 존재하면,  그 값을 **즉시 reservation station으로 복사**한다.  그러면 원래의 레지스터에는 더 이상 그 값이 필요 없으므로, **덮어써도 문제가 없다.**
    
만약 operand가 **아직 준비되지 않은 값이라면**,  reservation station은 해당 값을 아직 실행 중인 **실행 유닛(function unit)** 으로부터 **나중에 전달 받게 된다.** 이 경우, 해당 레지스터에 값을 직접 업데이트하지 않아도 되며,  **reorder buffer가 최종적으로 commit 시점에 레지스터를 갱신**하게 된다.

이 구조 덕분에 **같은 레지스터 이름을 여러 instruction이 중복해서 사용하더라도**, 실제로는 **다른 물리적인 공간에 값을 저장하며 의존성을 피할 수 있게 되는 것**,  즉, **register renaming**이 자연스럽게 이루어지는 것이다.

> 그냥 Reservation station에서 reorder을  수행하고, 그 결과는 원래 순서대로 반영된다.

---
## **Speculation**

**Load speculation**은 메모리 접근과 관련된 의존성을 **정확히 파악하기 어려운 경우**, 성능을 높이기 위해 **추정을 통해 먼저 load를 수행하는 방식**이다.
...
겉으로 보기에는 `$s1`과 `$t2`가 **다른 레지스터이므로 의존성이 없어 보인다.**  
하지만 실제로 **`$s1`과 `$t2`의 값이 같았다면**,  두 명령어는 **같은 메모리 주소를 접근**하게 되므로,  
`lw`가 `sw`보다 **먼저 수행되면 잘못된 값을 읽을 수도 있다.**

이러한 경우를 포함해서, **load 관련한 speculation을 많이 수행**한다.

**주소 자체(`$t2 + 10`)를 미리 예측**해서 접근할지 판단하거나, 경우에 따라 **로드할 값을 미리 예측**하기도 한다. **store가 아직 완료되지 않았음에도 load를 수행**하는 경우도 있으며, 또한 **load는 메모리 접근 지연으로 인해 한 사이클 안에 끝나지 않는 경우가 많기 때문에**,  **load에 대한 추정은 더 세밀하고 확실하게 이루어져야 한다.**

하지만 **이 모든 speculation은 임시적인 실행일 뿐**,  **최종 결과의 반영(commit)** 은 **항상 프로그램에 정의된 원래 순서대로** 이루어진다.  이렇게 해야 **정확한 프로그램 동작을 보장**할 수 있다.

>근데 이렇게 추정을 해도 결과 반영은 원래 순서대로!

---
## **Why Do dynamic Scheduling?**

왜 컴파일러가 코드만 보고 스케줄링하는 것으로는 충분하지 않을까?

가장 큰 이유는 **실행 환경이 매우 동적이기 때문**이다. 컴파일 시점에는 **실행 시점의 실제 레지스터 값**, **분기 결과(branch outcome)**, **메모리 접근의 실제 주소 충돌 여부**, **실제 하드웨어 구조(파이프라인 깊이, issue 수, forwarding 지원 등)** 이런 것들을 **정확하게 예측할 수 없다.**

특히 **브랜치 앞뒤로는 instruction을 묶기가 어려운데**,  이는 **분기 결과가 극단적으로 바뀔 수 있기 때문**이다. 예를 들어 예측이 틀리면 아예 잘못된 명령어를 실행하게 되므로, **컴파일러는 보수적으로 행동**할 수밖에 없다.

또한, **ISA(Instruction Set Architecture)** 는 같더라도, **실제 구현된 하드웨어 구조는 서로 다를 수 있다.**  따라서 **같은 코드라도 hazard 발생 조건이나 해결 방법이 달라질 수 있다.**

결론적으로, 컴파일러는 도움을 줄 수는 있지만, **모든 상황을 정적으로 예측하기에는 한계가 많고**,  
**정확한 제어는 실행 시점의 정보를 아는 CPU(하드웨어)가 직접 판단**하는 것이 더 유리하다.