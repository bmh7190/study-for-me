...
Hamming SEC Code는 failure가 덜 발생하도록 하는 방법 중 하나이다. 메모리는 완벽하지 않아서 시간이 지남에 따라 바뀌기도 한다. 이럴 때마다 복구하게 되면 cost 손해가 크기 때문에 1~2 바뀐건 그냥 넘어갈 수 있으면 좋겠다는 목표를 가진다. 

SEC code를 알기 전에 알아야 할 개념이 Hamming distance인데, 2개의 bit 패턴 중 얼마나 비트가 다른가? 를 나타낸다. 
