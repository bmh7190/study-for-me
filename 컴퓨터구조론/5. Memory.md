...
Cache에 들어갈 때, 어떤걸 교체 혹은 쫓아낼지 선택을 할 필요가 있다. 

Direct mapped는 어차피 미리 정해지기 때문에 선택할 필요 없다 
Set associative에서는 n개, 즉 way 만큼의 선택지가 있을 것이다. 어떤 것을 골라내는게 이상적일까? 앞으로 접근되지 않을 것을 예상해서 그걸 선택하는게 좋을 것이다. 하지만 우리는 미래를 모르기 때문에