...
이걸 스레드 하나만 놓고 보면, 메모리에서 `num_features` 간격으로 멀찍이 떨어진 위치들을 계속 접근하고 있으니, 언뜻 보기에는 메모리 접근 효율이 좋지 않아 보인다. 일종의 “뛰엄뛰엄(strided) 접근"처럼 보이기 때문이다.
...
이 방식에서는 스레드 하나가 이미지 전체의 픽셀을 순회하며 평균값을 빼주기 때문에, 스레드 단독으로만 보면 데이터가 **연속적인 메모리 구간**을 차례대로 읽게 된다. 즉, 한 스레드 입장에서 보면 이 방식이 훨씬 효율적으로 보일 수 있다.
