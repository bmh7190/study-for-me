...
- 각 스레드가 자신의 로컬 복사본에 gradient를 더해가고,
- 루프가 끝난 뒤 모든 스레드의 gradient들을 안전하게 더해 하나로 합친다.


즉, 여러 스레드가 동시에 `grad_bias` / `grad_weights`를 건드려도 race 없이 합쳐지게 함.
...
- 여기 사실은 `value_t* output = new value_t[num_classes];` 가 되어야 맞음 (지금은 포인터/배열 타입 오타 느낌)
