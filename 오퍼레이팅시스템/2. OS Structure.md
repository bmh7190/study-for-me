...
- **I/O Interrupt 발생 및 ISR 실행**
	I/O 디바이스가 데이터를 다 읽었다면 **인터럽트를 발생시켜 CPU에 알린다.** CPU는 현재 실행 중인 사용자 프로세스를 잠시 중단하고, **인터럽트 서비스 루틴(ISR, Interrupt Service Routine)** 을 실행한다. 여기서 ISR은 다 읽은 데이터를 실제 메모리로 옮기는 것이다.
