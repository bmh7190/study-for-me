...
(a) message passing의 경우 msgsnd(), msgrcv() 를 사용하는데 데이터를 복사해서 전달하기 때문에 대용량 데이터 전달에 분리하다. 하지만 동기화 관리가 필요하지 않다. 

(b) shared memory의 경우 : shared memory에 접근할 수 있는 주소(포인터)를 제공하여 대용