...
실제로 사용되기는 어렵다. 처음에 가정하는 자원의 수와 프로세스 수는 변동적이기도 하고, 

Max 값을 예측하는게 제일 어렵다. 모든 프로세스가 약속을 하고, 자신의 최대 자원 할당 값을 예측해야하는게 현실적으로 어렵다. 

자원 할당까지 wait을 해야 하는데, 계속 기다릴 수도 있다. 

너무 보수적이다. resource utilization이 낮다. 왜냐하면 safe state를 만들기 위해서 allocation 자체를 보수적으로 잡을 것이고, Max 값 자체도 낮게 잡을 것이기 때문에  