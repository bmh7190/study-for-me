...
우리가 사용하는 대부분의 OS 들은 synchoronization 기능을 가지고 있다. OS 자체가 여러 기능을 동시에 수행하기 때문이다. 

uniprocessor system에서는 공유 자원에 대한 접근을 할 때 criticial section 안에 들어가게 될 텐데 interrput 자체를 막는다. 

multiprocessor system에서는 spinlock을 사용한다. A 프로세서가 critical sectin