...
이 함수는 양쪽의 철학자들이 지금 밥을 먹고 있는 상태인지 확인하는 함수이다. 만약 양쪽 철학자 모두가 밥을 먹고 있지 않다면, 자신의 상태를 `EATING` 으로 바꾸고, 자기 자신을 깨운다. 
...
만약 내 상태가 지금 `EATING` 이 아니라면 즉 test 실패하면 자기 자신을 wait 하여 대기한다.
...
밥을 다 먹고 자기 자신의 상태를 `THINKING`으로 변경한다. 그리고 왼쪽 철학자와 오른쪽 철학자를 test 함수를 사용하면, 만약 `HUNGRY` 상태였고, 왼쪽 오른쪽 사람들이 안 먹고 있다면 그 철학자는 먹을 수 있는 상태로 깨어준다. 그럼 pickup 에서 self[i].wait에 의해 대기 중이었던 철학자는 깨어있겠죠?