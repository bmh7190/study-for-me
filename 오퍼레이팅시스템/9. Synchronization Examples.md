...
먹기 위해서는 즉 critical section 안에 들어가기 위해서는 젓가락 왼쪽과 오른쪽을 하나 씩 wait을 통해서 얻고, 다 먹었다면 signal을 통해서 제자리에 놓으면 된다.

이렇게 되면 Deadlock이 발생할 수 있다. 즉 하나씩 젓가락을 가지고 있고, 다른 한쪽을 서로에게 원하는 상황이 발생하게 된다. 예를 들어 모든 철학자가 오른쪽 젓가락만 가지고 있다면, 왼쪽 젓가락은 또 다른 철학자가 가지고 있을텐데, 그 젓가락을 풀어주려면, 다른 사람의 젓가락이 필요하다. 따라서 서로가 서로의 젓가락을 원하는 상황이기 때문에, 무한 굴레에 빠지게 된다.