...
wait(queue)를 통해 queue를 막고 들어가기 그 다음 rw_mutex 획득하고 들어가기!
...
이 코드는 기존 Reader 프로세스 구조에 **`queue` 세마포어를 추가한 버전**이다.  `queue`는 **Reader와 Writer 모두가 사용하는 공용 진입 세마포어**로,  **진입 순서를 제어하여 Writer starvation을 방지**하는 역할을 한다.

새로 들어오는 Reader는 가장 먼저 `wait(queue)`를 호출하여 **진입 순서를 보장**받는다.  
그 뒤에 `wait(mutex)`를 통해 `read_count`에 안전하게 접근하고,  
`read_count++` 이후, 만약 자신이 첫 번째 Reader라면 `rw_mutex`를 획득하여 **Writer의 접근을 차단**한다.

이후 `mutex`를 반납하고, `signal(queue)`를 호출하여 **다음 대기 중인 Reader 또는 Writer가 진입 가능하도록** 한다.

한편, **Writer가 진입하여 `wait(queue)`를 획득한 상태라면**,  
그 뒤에 오는 Reader들은 `wait(queue)`에서 **자동으로 대기 상태에 걸리게 되므로**,  
이미 진입하여 읽고 있는 Reader는 계속 진행하되,  
새로운 Reader는 **Writer보다 먼저 진입하지 못하도록 제한**된다.

읽기가 끝나면 `read_count--`를 통해 Reader 수를 줄이고,  
마지막 Reader라면 `signal(rw_mutex)`를 통해 Writer가 접근할 수 있도록 한다.