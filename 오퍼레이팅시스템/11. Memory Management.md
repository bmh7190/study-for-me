...
이를 해결하기 위한 장치가 바로 **TLB(Translation Lookaside Buffer)** 이다.  
TLB는 최근에 사용된 **page number → frame number 변환 정보를 캐시처럼 저장**하는 고속 메모리이다. 프로세스는 보통 **spatial locality** (인접한 주소들을 자주 접근) 성향을 보이므로, **최근 접근한 page 정보만 저장해도 상당히 많은 변환 요청을 처리**할 수 있다.
